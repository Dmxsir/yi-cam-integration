"""Single-attempt Phase 2C.3 runner for the POOL y291ga camera.
...
def classify(report: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "target": TARGET,
        "fresh_cloud_identity": "VERIFIED" if report.get("selected_camera") == TARGET and report.get("raw_model") == "83" and report.get("normalized_model") == "y291ga" and report.get("p2p_type") == 2 else "FAILED",
        ...
        "startup_burst_elapsed_ms": burst.get("elapsedMillis", "UNKNOWN"),
...
def parser() -> argparse.ArgumentParser:
    result = argparse.ArgumentParser(description="Phase 2C.3 single controlled POOL retry")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        # The historical Phase 2C host allowed another y291ga as fallback.
        # Phase 2C.3 is deliberately POOL-only. Repeating POOL in both slots
        # preserves the old preflight implementation while removing fallback.
        oracle.APPROVED_TARGETS = (TARGET, TARGET)
        material, preflight = oracle.fresh_cloud_preflight(args.timeout)
        if material.name != TARGET or material.raw_model != "83" or material.normalized_model != "y291ga":
