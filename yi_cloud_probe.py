...
@lru_cache(maxsize=1)
def _apk_model_mappings() -> tuple[dict[str, str], dict[str, str], frozenset[str]]:
    """Load serverModel/model mappings from the analyzed APK assets."""
    asset_dir = Path(__file__).with_name(".analysis") / "apk" / "assets" / "feature_config"
    documents: list[dict[str, Any]] = []
    if asset_dir.is_dir():
        paths = sorted(asset_dir.glob("config_*.json"))
        for path in paths:
            try:
        ...
        apk_path = Path(__file__).with_name("YI Home .apk")
        try:
            with zipfile.ZipFile(apk_path) as apk:
                ...
                for name in names:
                    try:
                        value = json.loads(apk.read(name).decode("utf-8"))
    ...
    for document in documents:
        server_model = _string(document.get("serverModel"))
        model = _string(document.get("model"))
        identifier = _string(document.get("identifier"))
        if not server_model or not model:
            continue
        models.add(model)
        if server_model == "10000":
            ...
            candidates.setdefault(server_model, set()).add(model)
    # Ambiguous duplicate IDs are intentionally left unresolved rather than guessed.
    mappings = {key: next(iter(values)) for key, values in candidates.items() if len(values) == 1}
    return mappings, identifiers, frozenset(models)


def normalize_model(raw_model: Any, did: Any = None) -> str:
    model = _string(raw_model)
    mappings, identifiers, known_models = _apk_model_mappings()
    if model == "10000":
        device_id = _string(did)
        return identifiers.get(device_id[:5], "UNKNOWN") if len(device_id) >= 5 else "UNKNOWN"
