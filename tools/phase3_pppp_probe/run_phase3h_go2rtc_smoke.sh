...
# API readiness alone does not prove the isolated RTSP listener is alive.
rtsp_ready=0
...
# Trigger the source through go2rtc's stream API as an additional diagnostic.
# The request may legitimately fail before a producer is ready, so it is not a
# test gate; its body/status are preserved locally for failure analysis.
API_HTTP="$(curl -sS --max-time 15 -o "$API_SNAPSHOT" -w '%{http_code}' \
    "http://127.0.0.1:${API_PORT}/api/streams?src=${STREAM}&video=all&audio=all" || true)"
...
if [[ "$PROBE_RC" -ne 0 ]]; then
    ...
    if [[ -s "$API_SNAPSHOT" ]]; then
        echo "--- go2rtc STREAM API SNAPSHOT ---" >&2
