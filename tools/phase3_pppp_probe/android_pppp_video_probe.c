...
static int magic_ok(const unsigned char *p) {
    return p[0] == 'Y' && p[1] == '3' && p[2] == 'V' && p[3] == '1';
}

typedef int (*pppp_read_fn)(int, unsigned char, unsigned char *, int *, int);

static int read_exact_pppp(pppp_read_fn pppp_read, int handle, unsigned char channel,
                           unsigned char *buffer, uint32_t count) {
    uint32_t done = 0;
    while (done < count) {
        int requested = (int)(count - done);
        int rc = pppp_read(handle, channel, buffer + done, &requested, -1);
...
static int drain_pppp(pppp_read_fn pppp_read, int handle, unsigned char channel, uint32_t count) {
    unsigned char scratch[DRAIN_CHUNK];
    while (count) {
        uint32_t chunk = count > DRAIN_CHUNK ? DRAIN_CHUNK : count;
        int rc = read_exact_pppp(pppp_read, handle, channel, scratch, chunk);
        if (rc < 0) return rc;
        count -= chunk;
    }
    return 0;
...
static int read_video_metadata(pppp_read_fn pppp_read, int handle, unsigned char channel) {
    unsigned char outer[8];
    unsigned char frame[24];
    int rc = read_exact_pppp(pppp_read, handle, channel, outer, sizeof(outer));
    if (rc < 0) return 80 + channel;

    uint32_t data_size = be32(outer + 4);
    if (outer[0] != TNP_VERSION || outer[1] != 1 || data_size < sizeof(frame) || data_size > 2U * 1024U * 1024U) {
        return 90 + channel;
    }

    rc = read_exact_pppp(pppp_read, handle, channel, frame, sizeof(frame));
    if (rc < 0) return 100 + channel;
    if (drain_pppp(pppp_read, handle, channel, data_size - sizeof(frame)) < 0) return 110 + channel;

    uint32_t codec = be16(frame + 0);
    uint32_t flags = frame[2];
    uint32_t sequence = be16(frame + 6);
    uint32_t width = be16(frame + 8);
    uint32_t height = be16(frame + 10);

    if (channel == 2) {
        put_str("channel2_codec_id="); put_u32(codec);
        put_str("channel2_is_i_frame="); put_u32(flags & 1U);
        put_str("channel2_sequence="); put_u32(sequence);
        put_str("channel2_width="); put_u32(width);
        put_str("channel2_height="); put_u32(height);
        put_str("channel2_payload_bytes="); put_u32(data_size - 24U);
    } else {
        put_str("channel3_codec_id="); put_u32(codec);
        put_str("channel3_is_i_frame="); put_u32(flags & 1U);
        put_str("channel3_sequence="); put_u32(sequence);
        put_str("channel3_width="); put_u32(width);
        put_str("channel3_height="); put_u32(height);
        put_str("channel3_payload_bytes="); put_u32(data_size - 24U);
    }

    if (codec != H264_CODEC_ID || width == 0 || height == 0) return 120 + channel;
    if (channel == 2 && (flags & 1U) == 0) return 130 + channel;
    return 0;
}

int main(int argc, char **argv) {
    const char *library = argc > 1 ? argv[1] : "/data/local/tmp/yi-phase3e2/libPPPP_API.so";
    ...
    put_str("phase3e2_video_probe=START\n");
    ...
    if (!did_len || !server_len || !key_len || did_len >= MAX_FIELD || server_len >= MAX_FIELD || key_len >= MAX_FIELD ||
        !unit1_len || !unit2_len || !unit3_len || !stop_len || unit1_len > MAX_UNIT || unit2_len > MAX_UNIT ||
        unit3_len > MAX_UNIT || stop_len > MAX_UNIT) {
    ...
    if (read_exact_fd(did, did_len) || read_exact_fd(server, server_len) || read_exact_fd(device_key, key_len) ||
        read_exact_fd(unit1, unit1_len) || read_exact_fd(unit2, unit2_len) || read_exact_fd(unit3, unit3_len) ||
        read_exact_fd(stop_unit, stop_len)) {
    ...
    if (!pppp_initialize || !pppp_deinitialize || !pppp_connect || !pppp_wakeup_connect || !pppp_connect_break ||
        !pppp_force_close || !pppp_write || !pppp_read) {
    ...
    handle_value = wakeup ? pppp_wakeup_connect(did, flag, udp_port, server, device_key)
    ...
    if (read_header_rc < 0 || read_size != 8) { result_code = 60; goto cleanup; }
    ...
    if (read_body_rc < 0 || read_size != (int)command_data_size) { result_code = 62; goto cleanup; }
    ...
    if (response_version != TNP_VERSION || response_command != CMD_SET_RESOLUTION_RESP || response_number != 1 || auth_result != 0) {
        result_code = 63; goto cleanup;
    }
    put_str("phase3e2_tnp_auth=PASS\n");

    put_str("stage=before_channel2_first_unit\n");
    result_code = read_video_metadata(pppp_read, handle_value, 2);
    if (result_code != 0) goto cleanup;
    put_str("stage=after_channel2_first_unit\n");

    put_str("stage=before_channel3_first_unit\n");
    result_code = read_video_metadata(pppp_read, handle_value, 3);
    if (result_code != 0) goto cleanup;
    put_str("stage=after_channel3_first_unit\n");
    put_str("phase3e2_video_channels=PASS\n");
    result_code = 0;
...
cleanup:
    ...
    if (result_code == 0) {
        put_str("phase3e2_video_probe=PASS\n");
        return 0;
    }
    put_str("phase3e2_video_probe=FAIL\n");
    put_str("phase3e2_failure_code="); put_u32((uint32_t)result_code);
    return result_code;
}
