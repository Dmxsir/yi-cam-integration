...
static void zero_bytes(void *ptr, unsigned long count) {
    unsigned char *p = (unsigned char *)ptr;
    for (unsigned long i = 0; i < count; i++) p[i] = 0;
}

int main(int argc, char **argv) {
    const char *library = argc > 1 ? argv[1] : "/data/local/tmp/yi-phase3d-check/libPPPP_API.so";
    unsigned char header[20];
    char did[MAX_FIELD];
    char server[MAX_FIELD];
    char device_key[MAX_FIELD];
    unsigned char init_string[1] = {0};
    unsigned char session_buffer[SESSION_BUFFER_SIZE] __attribute__((aligned(16)));
    int initialized = 0;
    int handle_value = -1;

    put_str("phase3d_check_probe=START\n");
    ...
    int (*pppp_check)(int, void *) =
        (int (*)(int, void *))dlsym(so, "PPPP_Check");
    ...
    if (!pppp_initialize || !pppp_deinitialize || !pppp_connect || !pppp_wakeup_connect ||
        !pppp_check || !pppp_connect_break || !pppp_force_close) {
    ...
    if (handle_value < 0) {
        ...
        put_str("phase3d_check_probe=CONNECT_FAILED\n");
        return 40;
    }

    /*
     * We intentionally do not interpret the private native session struct yet.
     * The Java wrapper proves PPPP_Check(handle, outSession), but not the exact
     * public C layout. A large aligned scratch buffer lets this phase validate
     * the call safely without guessing field offsets or exposing peer data.
     */
    zero_bytes(session_buffer, sizeof(session_buffer));
    put_str("session_buffer_bytes=512\n");
    put_str("stage=before_check\n");
    int check_rc = pppp_check(handle_value, session_buffer);
    put_str("PPPP_Check_rc_hex="); put_hex32((uint32_t)check_rc);
    put_str("stage=after_check\n");
    ...
    if (check_rc != 0) {
        put_str("phase3d_check_probe=CHECK_FAILED\n");
        return 42;
    }
    if (break_rc != 0 || close_rc != 0 || deinit_rc != 0) {
        put_str("phase3d_check_probe=CLEANUP_FAILED\n");
        return 41;
    }

    put_str("phase3d_check_probe=PASS\n");
    return 0;
}
