...
TARGET_DIR="$RUNTIME/data/local/tmp/yi-phase3e2"
...
"$CC" -nostdlib -fPIE -pie -fno-stack-protector -fno-builtin \
    ...
    -o "$TARGET_DIR/android_pppp_video_probe" \
    "$SRC_DIR/android_start.S" \
    "$SRC_DIR/android_pppp_video_probe.c" \
    -L"$RUNTIME/system/lib64" \
    -Wl,-l:libdl.so \
    -Wl,-l:libc.so \
    -Wl,-l:ld-android.so

chmod +x "$TARGET_DIR/android_pppp_video_probe"

echo "=== PHASE 3E2: PHONELESS BIONIC VIDEO CHANNEL PROBE ==="
echo "The proven PPPP/TNP startup is reused unchanged for the exact warehouse camera."
echo "After successful 4882 authentication, one complete TNP unit is consumed from channel 2 and one from channel 3."
echo "Only frame metadata is printed; compressed video bytes are discarded in this smoke phase."
echo "No ADB or USB phone is used."
echo
echo "--- PROBE ---"
file "$TARGET_DIR/android_pppp_video_probe"
readelf -l "$TARGET_DIR/android_pppp_video_probe" | grep -F 'Requesting program interpreter' || true
readelf -d "$TARGET_DIR/android_pppp_video_probe" | grep NEEDED || true

echo
echo "--- RUN WITHOUT ADB ---"
"$PYTHON" "$SRC_DIR/run_phase3e2_video.py" \
    --env-file "$ENV_FILE" \
    --runtime "$RUNTIME" \
