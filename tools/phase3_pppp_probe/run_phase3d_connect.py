...
def main() -> int:
    ...
    try:
        material, preflight = oracle.fresh_cloud_preflight(timeout=10.0)
        if material.name != args.target:
            raise RuntimeError(
                f"Exact Phase 3D target {args.target!r} is not the selected online camera; fallback refused"
            )
        if (
            material.raw_model != "83"
            or material.normalized_model != "y291ga"
            or preflight.get("p2p_type") != 2
            or preflight.get("cloud_online") is not True
        ):
            raise RuntimeError("Phase 3D exact-target identity check failed")
