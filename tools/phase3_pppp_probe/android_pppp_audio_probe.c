...
static int magic_ok(const unsigned char *p) {
    return p[0] == 'Y' && p[1] == '3' && p[2] == 'A' && p[3] == '1';
}
...
static int read_audio_unit(pppp_read_fn pppp_read, int handle) {
    ...
    put_str("channel1_tnp_version="); put_u32(outer[0]);
...
int main(int argc, char **argv) {
    const char *library = argc > 1 ? argv[1] : "/data/local/tmp/yi-phase3e3/libPPPP_API.so";
    ...
    put_str("phase3e3_audio_probe=START\n");
    ...
    if (!did_len || !server_len || !key_len || did_len >= MAX_FIELD || server_len >= MAX_FIELD || key_len >= MAX_FIELD ||
        ...
        unit3_len > MAX_UNIT || stop_len > MAX_UNIT) return 11;
    ...
    if (read_exact_fd(did, did_len) || read_exact_fd(server, server_len) || read_exact_fd(device_key, key_len) ||
        ...
        read_exact_fd(stop_unit, stop_len)) return 12;
    ...
    if (!pppp_initialize || !pppp_deinitialize || !pppp_connect || !pppp_wakeup_connect ||
        !pppp_connect_break || !pppp_force_close || !pppp_write || !pppp_read) return 21;
    ...
    put_str("phase3e3_tnp_auth=PASS\n");

    put_str("stage=before_channel1_first_unit\n");
    result_code = read_audio_unit(pppp_read, handle_value);
...
cleanup:
    ...
    if (result_code == 0) {
        put_str("phase3e3_audio_probe=PASS\n");
        return 0;
    }
    put_str("phase3e3_audio_probe=FAIL\n");
    return result_code;
}
