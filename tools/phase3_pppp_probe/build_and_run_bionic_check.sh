...
TARGET_DIR="$RUNTIME/data/local/tmp/yi-phase3d-check"
...
"$CC" -nostdlib -fPIE -pie -fno-stack-protector -fno-builtin \
    ...
    -o "$TARGET_DIR/android_pppp_check_probe" \
    "$SRC_DIR/android_start.S" \
    "$SRC_DIR/android_pppp_check_probe.c" \
    -L"$RUNTIME/system/lib64" \
    -Wl,-l:libdl.so \
    -Wl,-l:libc.so \
    -Wl,-l:ld-android.so

chmod +x "$TARGET_DIR/android_pppp_check_probe"

echo "=== PHASE 3D2: PHONELESS BIONIC PPPP_CHECK PROBE ==="
echo "Connect is performed exactly as in the proven Phase 3D path."
echo "PPPP_Check is then called with an oversized aligned opaque output buffer."
echo "No session fields, peer addresses, TNP commands, passwords, or video are logged."
echo "No ADB or USB phone is used."
echo

echo "--- PROBE ---"
file "$TARGET_DIR/android_pppp_check_probe"
readelf -l "$TARGET_DIR/android_pppp_check_probe" | grep -F 'Requesting program interpreter' || true
readelf -d "$TARGET_DIR/android_pppp_check_probe" | grep NEEDED || true

echo

echo "--- RUN ---"
"$PYTHON" "$SRC_DIR/run_phase3d_check.py" \
    --env-file "$ENV_FILE" \
    --runtime "$RUNTIME" \
