...
- the proven Bionic guest runtime;
- the Phase 3G native media worker artifacts;
- the proven `PPPP_CheckDevOnline` worker/library;
- a secret-safe runtime manifest with hashes of critical files.

It does **not** copy `.env.local`, YI credentials, backend tokens, runtime policy or capability state. Generated `yi_home/rootfs` content is ignored by Git.
...
/data/                                persistent App state
```

On first App start `/run.sh` generates `/data/backend-api-token` with mode 0600, starts the backend on internal TCP 8099, and publishes Supervisor discovery service `yi_home` containing only internal host/port/API metadata and the internal token.
